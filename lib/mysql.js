...
module.exports = function(config, rbac) {
    ...
    _this.queryuser = function(id) {
        ...
        if (!id) {
            ...
            _this.query('select ' + rbac.userTrole + ' from ' + rbac.user + ' where id = ' + id).then(function(data) {
    ...
    _this.querynode = function(idarr) {
        ...
        if (idparam.length <= 0) {
            ...
            idparam = logic.unique(idparam.join(',').split(',')).join(',');
            _this.query('select ' + rbac.nodeTroute + ' from ' + rbac.node + ' where id in (' + idparam + ')').then(function(data) {
                deferred.resolve(data.results);
            }, function(error) {
                deferred.reject(error);
            });
        ...
        _this.queryuser(userid)
            ...
            .then(function(data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    idparam.push(data[i][rbac.nodeTroute]);
                }
                if (idparam.length > 0) {
                    // 去重
