...
var addnode = function(param, callback) {
    query('insert into node(name, route, method, tag) values(?, ?,?, ?)', param, function(error, results, fields) {
        if (error) {
            throw error;
        }
        if (callback) {
...
var eachnode = function(callback) {
    query('select node.id as id, node.name as name, node.route as route, node.method as method, node_tag.name as tag_name from node left join node_tag on node.tag = node_tag.id', function(error, results, fields) {
        if (error) {
            throw error;
        }
        return callback(results);
